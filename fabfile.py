...
REQUIREMENTS_FILE_PATH = getattr(settings, "REQUIREMENTS_FILE_PATH", None)
...
def check_local_root(func):
    ...
    Wraps functions that require LOCAL_PROJECT_ROOT so don't have to repeat
    the code in each
    """
    @wraps(func)
    def wrapper(*args, **kwargs):
        if not LOCAL_PROJECT_ROOT or not os.path.isdir(LOCAL_PROJECT_ROOT):
...
def check_remote_root(func):
    ...
    Wraps functions that require REMOTE_PROJECT_ROOT so don't have to repeat
    the code in each
    """
    @wraps(func)
    def wrapper(*args, **kwargs):
...
def update_requirements():
    ...
    """
    with cd(REMOTE_PROJECT_ROOT):
        run("pip install -r {0}".format(REQUIREMENTS_FILE_PATH))
...
def deploy(amend=False):
    ...
    if confirm("Do we need to update css or js, syncdb, migrate, or update "
        ...
        migrate = confirm("Migrate database?")
